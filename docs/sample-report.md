<!-- This path is preserved deliberately. The NuGet package pages for 0.1.0 and 0.1.1 link here,
     and nuget.org holds published versions immutable, so the link cannot be corrected. Moving
     this file would break it permanently. New samples go under docs/samples/<estate>/. -->

# Sample report

A MigrationScan Markdown report, generated from the Woodgrove Bank fixture: a fictional
four-project retail banking estate that carries every shape a report can produce, from an
architectural blocker to a project the tool cannot assess.

The [estate write-up](samples/woodgrove-banking/) explains what each number means and links the
raw console and JSON output.
...
- **Estimated effort:** 29.3–88 engineer-days, plus 1 item requiring an architectural decision before they can be estimated
...
| [MIG1006](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG1006.md) | Medium | Certain | Medium | `Woodgrove.Interop/Woodgrove.Interop.csproj:22` | Project 'Woodgrove.Interop' has a COM reference ('FabrikamImagingLib'). COM interop works on modern .NET only when targeting Windows (net-windows); it is a Windows lock-in and unavailable elsewhere. |
...
| [MIG4002](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG4002.md) | High | Probable | Small | `Woodgrove.Interop/TerminalProfile.cs:15` | Accesses the Windows Registry (Microsoft.Win32.Registry). The registry does not exist on non-Windows platforms; the call throws PlatformNotSupportedException there. |
| [MIG4002](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG4002.md) | High | Probable | Small | `Woodgrove.Interop/TerminalProfile.cs:23` | Accesses the Windows Registry (Microsoft.Win32.Registry). The registry does not exist on non-Windows platforms; the call throws PlatformNotSupportedException there. |
| [MIG4003](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG4003.md) | High | Probable | Medium | `Woodgrove.Interop/BranchHostInventory.cs:2` | Uses WMI (System.Management). On modern .NET this is Windows-only (via the System.Management package) and throws PlatformNotSupportedException elsewhere. |
| [MIG4013](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG4013.md) | Medium | Probable | Small | `Woodgrove.Interop/ChequeScannerInterop.cs:12` | P/Invokes the Windows system library 'kernel32.dll' via [DllImport]. This works on modern .NET only when targeting Windows (net-windows); it is a Windows lock-in and won't run cross-platform. |
| [MIG4013](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG4013.md) | Medium | Probable | Small | `Woodgrove.Interop/ChequeScannerInterop.cs:16` | P/Invokes the Windows system library 'advapi32.dll' via [DllImport]. This works on modern .NET only when targeting Windows (net-windows); it is a Windows lock-in and won't run cross-platform. |
...
| [MIG2001](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG2001.md) | High | Certain | Medium | `Woodgrove.Web/packages.config:3` | Package 'Microsoft.AspNet.Mvc' has no version that supports net10.0. ASP.NET MVC 5 runs only on .NET Framework. Consider: ASP.NET Core MVC. |
| [MIG2001](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG2001.md) | High | Certain | Medium | `Woodgrove.Web/packages.config:4` | Package 'Microsoft.AspNet.Web.Optimization' has no version that supports net10.0. System.Web bundling/minification runs only on .NET Framework. Consider: A build-time bundler or ASP.NET Core static asset pipeline. |
...
| `Woodgrove.Interop/Woodgrove.Interop.csproj` | 9 | 11.5–35 | 0 |
...
**[MIG1006](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG1006.md) · COM reference or interop assembly**

COM interop is supported on modern .NET only when targeting Windows (net-windows), so regenerate the interop assembly there. It is a Windows lock-in: if the app must run cross-platform there is no COM support off Windows, so the component must be replaced. Very old or ActiveX components may also need updating even on Windows.
...
**[MIG4002](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG4002.md) · Windows Registry access**

The Windows Registry does not exist on other platforms; Microsoft.Win32.Registry throws PlatformNotSupportedException there. Move configuration/state to a cross-platform store (config files, environment variables, a database).

**[MIG4003](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG4003.md) · System.Management / WMI**

WMI (System.Management) is Windows-only on modern .NET and throws PlatformNotSupportedException elsewhere. If the app must run cross-platform, replace WMI queries with platform-appropriate APIs.

**[MIG4013](https://github.com/matt-williams-dev/MigrationScan/blob/main/docs/rules/MIG4013.md) · P/Invoke to a Windows system DLL**

P/Invoke to a Windows system library (kernel32/user32/advapi32/...) works on modern .NET when targeting Windows (net-windows). It is a Windows lock-in: for cross-platform, guard the call with OperatingSystem.IsWindows() and provide a non-Windows path, or replace it with a cross-platform .NET API.
