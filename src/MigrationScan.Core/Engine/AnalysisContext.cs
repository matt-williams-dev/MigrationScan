...
public sealed class AnalysisContext
{
    ...
    private IReadOnlyList<SourceFile> LoadSourceFiles() =>
        EnumerateProjectFiles()
            .Where(IsSourceFile)
            .Select(path => new { Relative = ToRelative(path), Path = path, Text = File.ReadAllText(path) })
            .Where(x => !IsGeneratedSource(x.Path, x.Text)) // don't analyze (or drown in) generated code
            .OrderBy(x => x.Relative, StringComparer.Ordinal)
            .Select(x => new SourceFile(x.Relative, ParseSource(x.Text, x.Path, x.Relative)))
            .ToList();
    ...
    // Skips tool-generated source (WCF/EF proxies, designer files). Migration regenerates these
    // rather than hand-editing them, and they otherwise flood the results (e.g. a svcutil
    // Reference.cs fully-qualifies System.ServiceModel thousands of times).
    private static bool IsGeneratedSource(string path, string text)
    {
        ...
        // Standard "<auto-generated>" header, checked near the top of the file.
        ReadOnlySpan<char> head = text.AsSpan(0, Math.Min(text.Length, 800));
        return head.Contains("<auto-generated", StringComparison.OrdinalIgnoreCase);
    }

    private static SyntaxTree ParseSource(string text, string path, string relativePath) =>
        Path.GetExtension(path).Equals(".vb", StringComparison.OrdinalIgnoreCase)
